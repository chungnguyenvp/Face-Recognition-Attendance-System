...
def face_file(face_id: int, user=Depends(current_user)):
    with get_db() as db:
        row = file_repository.get_student_face_file(db, face_id)
    if not row:
...
def access_log_evidence_file(log_id: int, user=Depends(current_user)):
    with get_db() as db:
        row = file_repository.get_access_log_evidence_file(db, log_id)
    if not row:
...
def alert_evidence_file(alert_id: int, user=Depends(current_user)):
    ...
    with get_db() as db:
        row = file_repository.get_alert_evidence_file(db, alert_id)
    if not row:
