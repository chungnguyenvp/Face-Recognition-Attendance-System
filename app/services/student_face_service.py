...
from app.services.face_capacity_service import MAX_FACE_SAMPLES
...
MAX_FACE_EMBEDDINGS_PER_STUDENT = MAX_FACE_SAMPLES
...
def delete_student_faces(db, student_id: int) -> list[str]:
    image_paths = student_face_repository.list_student_face_image_paths(db, student_id)
...
def trim_old_face_embeddings(
    ...
    count = student_face_repository.count_student_faces(db, student_id)
    overflow = max(count - max_count, 0)
    if overflow:
        rows = student_face_repository.list_oldest_student_faces(db, student_id, overflow)
        ids = [row["id"] for row in rows]
        image_paths = [row["image_path"] for row in rows if row["image_path"]]
        student_face_repository.delete_student_faces_by_ids(db, ids)
        return max_count, image_paths
    return count, []


def duplicate_warnings(db, student_id: int, embeddings: list) -> list[dict]:
    rows = student_face_repository.list_active_face_embeddings_except_student(db, student_id)
    warnings = {}
