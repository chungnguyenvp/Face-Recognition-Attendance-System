...
PRIVATE_FACE_REQUEST_DIR = PRIVATE_STORAGE_ROOT / "face_requests"
PRIVATE_REPORT_DIR = PRIVATE_STORAGE_ROOT / "student_reports"
...
def ensure_private_storage() -> None:
    ...
    PRIVATE_FACE_REQUEST_DIR.mkdir(parents=True, exist_ok=True)
...
def face_request_relative_path(request_key: str, filename: str) -> str:
    return f"face_requests/{Path(request_key).name}/{Path(filename).name}"


def report_relative_path(report_id: int, version_no: int, filename: str) -> str:
    return f"student_reports/report_{int(report_id)}/version_{int(version_no)}/{Path(filename).name}"
...
def normalize_private_path(stored_path: str | None, kind: str) -> str | None:
    ...
    expected_prefixes = {
        "face": "faces/",
        "evidence": "evidence/",
        "face_request": "face_requests/",
        "report": "student_reports/",
    }
    expected_prefix = expected_prefixes.get(kind)
...
def _migrate_student_faces(db) -> None:
    rows = private_storage_repository.list_legacy_student_faces(db)
    for row in rows:
        private_path = normalize_private_path(row["image_path"], "face")
        if not private_path:
            continue
        _move_legacy_file(row["image_path"], private_path, "face")
        private_storage_repository.update_student_face_path(db, row["id"], private_path)


def _migrate_evidence_paths(db, table_name: str) -> None:
    rows = private_storage_repository.list_legacy_evidence_paths(db, table_name)
    for row in rows:
        ...
        private_storage_repository.update_evidence_path(db, table_name, row["id"], private_path)
